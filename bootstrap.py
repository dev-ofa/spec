...
def build_agents_block(spec_relative_path: str, *, language: str) -> str:
    if language == "zh":
        return (
            ...
            f"- 将 `{spec_relative_path}/` 视为只读规范来源，不要把它当作当前仓库的本地规范目录来直接修改。\n"
            f"- 如果当前任务需要新增或调整本仓库规则，应在 `{spec_relative_path}/` 之外的合适位置落地；如果发现 spec 本身需要变更，应单独回到 spec 源仓库处理。\n"
    ...
    return (
        ...
        f"- Treat `{spec_relative_path}/` as a read-only spec source; do not treat it as this repository's local rules directory and edit it in place.\n"
        f"- If this repository needs local rules, add them outside `{spec_relative_path}/`; if the spec itself needs changes, send them back to the spec source repository separately.\n"
