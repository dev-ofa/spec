...
本指南描述 Python 项目在目录分层、包/环境管理与基础命令方面的推荐做法，用于把 [spec](../../spec/README.md) 落地到 Python 实现中。
...
必须目标：可复现安装（开发机与 CI 一致），并能明确区分“运行时依赖”和“开发依赖”。
...
- Python 版本管理（择一即可）：
  - `mise` / `asdf` / `pyenv`
- 虚拟环境：
  - 必须使用虚拟环境（工具自带或 `venv`），避免污染系统 Python
