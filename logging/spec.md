...
#### 必须
- 服务必须明确定义请求生命周期日志的记录边界
- 每个进入该边界的请求必须记录一条开始日志与一条结束日志
- 日志必须包含 trace_id 与 request_id
- 链路上下文、请求结果和错误摘要必须使用结构化字段记录，不得只拼接在 message 中
- 如果认证拒绝、路由未命中、前置处理失败或限流等请求没有进入该边界，必须由对应处理层记录独立的拒绝或失败日志，并携带 `trace_id`、`request_id`、`method`、`path`、`status_code` 与错误摘要

#### 记录边界
- 请求生命周期日志可以记录在服务入口后的统一访问层，也可以记录在统一接口处理层
- 选择服务入口后的统一访问层时，生命周期日志表示更完整的服务内请求处理周期；请求参数与响应参数可以使用独立日志记录，并通过 `trace_id` 与 `request_id` 关联
- 选择统一接口处理层时，生命周期日志表示接口业务处理周期；该模式可以将链路上下文、请求参数、响应参数、结果和耗时收敛到接口处理相关日志中，但必须明确不覆盖的前置阶段，并为前置拒绝或失败提供独立日志
- 同一服务内不得同时记录两套语义相同的请求生命周期日志；如果存在多层日志，必须使用不同事件名区分生命周期日志、参数诊断日志和错误主记录
...
- 请求处理出现错误时，应由入口层、前置处理层或统一错误处理层生成一条错误主记录
- 错误主记录应包含 `trace_id`、`request_id`、`method`、`path`、`status_code`、`code`、`duration_ms` 与错误摘要；是否包含堆栈取决于错误类别和脱敏要求
- 错误主记录不得替代请求结束日志；同一个错误也不得在中间层和入口层重复记录

### 请求与响应参数日志

- 服务可以记录请求参数和响应参数，用于问题定位、联调或审计
- 请求参数和响应参数可以与请求开始日志、请求结束日志合并记录，也可以作为独立的参数诊断日志记录
- 是否合并由实现方根据日志可读性、排障需求、参数体积和采集成本决定；规范不要求请求参数、响应参数与生命周期日志必须在同一条日志中
- 独立参数诊断日志必须携带 `trace_id` 与 `request_id`，并使用稳定事件名与生命周期日志区分
- 参数日志应记录经过解析、裁剪和脱敏后的字段；不得为了排障直接记录完整原始请求体、完整原始响应体或敏感凭证
- 默认情况下，参数日志应支持关闭、采样或按接口白名单启用；高频接口、大响应接口和包含敏感数据的接口不应无条件记录完整参数
- 当请求参数或响应参数本身会影响路由、权限、租户隔离、计费、幂等或关键业务判断时，可以将必要字段补充到生命周期日志或错误主记录中
...
### 级别建议
- 正常结束使用 INFO
- 预期内失败或业务失败通常使用 WARN；如果该失败是高频、无需排查且不影响系统健康的正常业务结果，可以使用 INFO
- 非预期错误或系统错误使用 ERROR
