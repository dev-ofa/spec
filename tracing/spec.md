...
- `request_id` **必须** 使用 `req_<YYYYMMDDHHMMSS>_<suffix>` 格式，例如 `req_20260420153045_9F1C2A3B4D5E6F708192`
- `request_id` 中的时间前缀 **必须** 使用 Local Time，并在同一条链路内保持统一
- `request_id` 中的 `<suffix>` **必须** 由加密安全随机数生成，并编码为 `20` 位大写十六进制字符串
- `request_id` 的 `<suffix>` **不得** 依赖中心发号器、机器 IP、进程号、线程号或普通伪随机数生成器
- `request_id` 的固定前缀 **必须** 为小写 `req_`；除该固定前缀外，生成结果 **必须** 仅包含数字、大写十六进制字符与下划线，便于在不同协议、日志系统与网关中稳定传递
